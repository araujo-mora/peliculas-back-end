...
namespace Back_end.Controllers
{
    ...
    public class CinesController: ControllerBase
    {
        ...
        public async Task<ActionResult<List<CineDTO>>> Get([FromQuery] PaginacionDTO paginacionDTO)
        {
            ...
            var cines = await queryable.OrderBy(x => x.Nombre).Page(paginacionDTO).ToListAsync();
        ...
        [HttpPut("{id:int}")]
        public async Task<ActionResult> Put(int Id, [FromBody] CrearCineDTO crearCineDTO)
        {
            var cine = await context.Cines.FirstOrDefaultAsync(x => x.Id == Id);
            if (cine == null)
            {
            ...
            cine = mapper.Map(crearCineDTO, cine);
            await context.SaveChangesAsync();
            return NoContent();
        }
